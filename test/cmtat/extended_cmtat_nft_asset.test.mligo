...
let test_grant_role_failure_not_admin =
  ...
  let () = string_failure r CMTAT_nft_asset.Token.AUTHORIZATIONS.Errors.not_ruler in
...
let test_revoke_role_failure_not_ruler =
  ...
  let () = Test.set_source op2 in
  let flag_burner : CMTAT_nft_asset.Token.AUTHORIZATIONS.role = MINTER in
...
let test_schedule_snapshot_failure_before_next_scheduled =
  ...
  let () = string_failure r CMTAT_nft_asset.Token.SNAPSHOTS.Errors.before_next_scheduled in
  ()
